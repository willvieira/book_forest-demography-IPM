...
# Code to generate fig for presentation
...
labels <- c(
  # `900` = "Z[infinity]==90",
  `1200` = "Maximum expected size = 120"
)
...
dt |>
  ...
  facet_wrap(
    ~Linf,
    labeller = labeller(Linf = labels)
  ) +
  geom_path(linewidth = 1) +
  theme_classic() +
  labs(
    x = expression(Delta~'time'),
    y = 'Size (cm)',
    color = 'ln(Annual\ngrowth rate)'
  ) +
  theme(
  ...
  scale_y_continuous(labels = scales::number_format(accuracy = 0.1)) ->
p
ggsave('../talk_PhDdefence/images/growth_example.png', p, width = 5.5, height = 3)
```

::: {.callout-note collapse="true"}
## Formal properties (Lean 4) {.unnumbered}

The following properties of the von Bertalanffy growth map are machine-checked proofs verified with [Lean 4](https://lean-lang.org/) and [Mathlib](https://leanprover-community.github.io/mathlib4_docs/):

| Property | Statement |
|---|---|
| Fixed point | $\zeta_\infty$ is the unique fixed point of the map |
| Contraction | Distance to $\zeta_\infty$ shrinks by exactly $e^{-r \Delta t}$ per step |
| Closed form | $\mu^{[n]}(x) = x \cdot e^{-r \Delta t \cdot n} + \zeta_\infty (1 - e^{-r \Delta t \cdot n})$ |
| Convergence | Iterates converge to $\zeta_\infty$ from any starting size when $r, \Delta t > 0$ |

Source: [`lean/ForestIPM/Growth.lean`](https://github.com/willvieira/forest-IPM/blob/master/lean/ForestIPM/Growth.lean)
:::
...
p
ggsave('../talk_PhDdefence/images/survival_example.png', p, width = 5.5, height = 3)
```

::: {.callout-note collapse="true"}
## Formal properties (Lean 4) {.unnumbered}

The following properties of the survival model are machine-checked proofs verified with [Lean 4](https://lean-lang.org/) and [Mathlib](https://leanprover-community.github.io/mathlib4_docs/):

| Property | Statement |
|---|---|
| Positivity | $\psi^{\Delta t} > 0$ for all inputs |
| Upper bound | $\psi^{\Delta t} < 1$ for all $\Delta t > 0$ |
| Bounded | $\psi^{\Delta t} \in (0, 1)$ for all $\Delta t > 0$ |
| Monotonicity | Survival probability is strictly decreasing in $\Delta t$ |

Source: [`lean/ForestIPM/Survival.lean`](https://github.com/willvieira/forest-IPM/blob/master/lean/ForestIPM/Survival.lean)
:::
...
expand_grid(
  ...
  mutate(
    rho_label = paste0('annual survival rate = ', as.character(round(exp(-exp(rho)), 3))),
    phi_label = case_match(
      phi,
      -9 ~ '1e-4',
      -8 ~ '3e-4',
...
p
ggsave('../talk_PhDdefence/images/recruit_example.png', p, width = 7, height = 3)
...
The following properties of the recruitment model are machine-checked proofs verified with [Lean 4](https://lean-lang.org/) and [Mathlib](https://leanprover-community.github.io/mathlib4_docs/):

| Property | Statement |
|---|---|
| Miss probability bounds | Annual miss probability $p \in (0, 1)$ |
| Cumulative factor | $(1 - p^{\Delta t}) / (1 - p) > 0$ for $p \in (0,1)$ and $\Delta t > 0$ |
| Rate positivity | Ingrowth rate $\phi \cdot A \cdot \frac{1 - p^{\Delta t}}{1 - p} > 0$ |
| Recruit size PDF | Gaussian density of recruit sizes is non-negative (and $> 0$ when $\sigma > 0$) |
| Kernel non-negativity | Full recruitment kernel $F(z', z) \geq 0$ |

Source: [`lean/ForestIPM/Ingrowth.lean`](https://github.com/willvieira/forest-IPM/blob/master/lean/ForestIPM/Ingrowth.lean)
:::
