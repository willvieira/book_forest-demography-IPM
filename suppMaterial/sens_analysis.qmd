...
However, most of the methods quantify the local sensitivity each parameter is evaluated separately while holding all others constant [@Saltelli2019].
This approach can overlook the obscure parameter interactions that are often common in complex models.
Furthermore, because of the high dimensionality of IPM due to a large number of parameters, these methods can quickly become computationally expensive.
...
#| fig-width: 8
#| fig-height: 6
#| fig-cap: "Ternary plot decribing the distribution of importance among the growth, survival, and recruitment models. Color represetns the level of shade tolerance [@burns1990silvics]."
...
#| fig-width: 8
#| fig-height: 4
#| fig-cap: "Correlation between the survival and recruitment relative importance across the 31 species, climate and competition conditions. Species points are grouped by a Multivariate Normal Density function with probability of 90%."
...
#| fig-width: 8
#| fig-height: 4
#| fig-cap: "Distribution of relative importance between climate and competition covariates according to Random Forest, the respective $R^2$. The more species are to the right of the panel, the more climate is important relative to competition. Color represetns the level of shade tolerance [@burns1990silvics]."
...
This choice is based on the common invasion growth rate metric, or the population growth rate when rare, an important measure for quantifying population persistence [@lewontin1969].
...
Also, the importance attributed to the survival model for certain species at the center and hot temperature conditions, shifted toward the recruitment model.
Although we observed this shift, the overall patterns remained similar to those discussed earlier.
The only exeption was for the distribution of relative importance between climate and competition [@fig-climVsComp], where many species had an increase in the importance of competition relative to climate.
