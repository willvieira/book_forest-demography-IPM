...
# main directory where parameters are saved (TODO)
model_dir <- file.path('..', '..', 'TreesDemography', 'output_sim_processed')
...
sigma_plot |>
  group_by(sp_id, model, vital_rate) |> 
...
sigma_plot |>
  ...
  geom_jitter(
    data = sigma_mean,
    aes(mean_par, model, color = sp_id),
    alpha = 0.8,
    height = 0.15
  ) +
...
R2 |>
  group_by(species_id, model, vital_rate) |> 
...
R2 |>
  ...
  geom_jitter(
    data = R2_mean,
    aes(mean_r2, model, color = species_id),
    alpha = 0.8,
    height = 0.15
  ) +
  geom_boxplot(fill = 'transparent') +
...
#| fig-width: 6
#| fig-height: 4
#| fig-cap: "Posterior distribution of Mean Squared Error (MSE) across the 32 tree species as models become more complex."
...
MSE |>
  ...
  geom_jitter(
    data = MSE_mean,
    aes(MSE_mean, model, color = species_id),
    alpha = 0.8,
    height = 0.15
  ) +
  geom_boxplot(fill = 'transparent') +
...
#| fig-width: 8
#| fig-height: 4
#| fig-cap: "Comparing the posterior distribution of sensitivity, specificity, and accuracy across the 32 tree species as models become more complex. Each colored dot represents the species' average posterior distribution."
...
acc |>
  group_by(species_id, model, name) |> 
...
acc |>
  ...
  geom_jitter(
    data = acc_mean,
    aes(acc_mean, model, color = species_id),
    alpha = 0.8,
    height = 0.15
  ) +
  geom_boxplot(fill = 'transparent') +
...
#| fig-width: 8
#| fig-height: 6
#| fig-cap: "Boxplot shows the LOO-CV compare between the competing models based on the difference in the expected log pointwise predictive density (ELPD_diff) across the 32 tree species. The sd_diff is the standard error of the ELPD difference between the model and the reference model (ELPD_diff equal to zero)."

loo_tb |>
  select(species_id, vital_rate, model, elpd_diff, se_diff) |>
...
```{r load_survival,echo=Echo,eval=Eval,cache=Cache,warning=Warng,message=Msg}
vital_rates |>
  set_names() |>
  map(~dir(file.path(model_dir, .x))) |>
  map(~.x[.x != 'intcpt']) ->
