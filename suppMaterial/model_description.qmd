...
The growth function describes how individual trees increase in size, while the survival function determines the probability to stay alive throughout their lifespan.
...
Because of trees are long-lived with a wide range of possible sizes among trees, these three vital rates should be size-dependent.
However, due to a lack data regarding the recruitment process, we will define a general recruitment rate that remains independent of the distribution of size among individuals within a population.

In this chapter, I will describe the concept and ecological interpretations underlying each of the three demographic models.
...
All the model equations described bellow are species-dependent, but we omit the notation for the species for clarity.
...
The rationale behind this equation is that growth rate exponentially decreases with size, converging to zero as size approaches $\zeta_{\infty}$ (@fig-VBModel).
...
&\mu = z_{i,t + \Delta t}|dbh_{i,t}, \Gamma, \zeta_{\infty}
\end{align*}
$$

Where $N$ is the normal distribution and $dbh_i$ is the observed size of an individual at two consecutive measurements.
The prior distribution of the parameters are defined as follows:

$$
\begin{align*}
&\Gamma \sim N(-3.5, 1) \\
&\zeta_{\infty} \sim N(1000, 80) \\
&\sigma \sim N(0, 1.5)
\end{align*}
$$
...
The rationale of this model is that the survival probability ($1 - p_i$) increases with the longevity parameter $\psi$ but it is compenseted exponentially with the increase in time $\Delta t$ (@fig-mortModel).
...
However, due to inconsistent protocols in recording seedlings, saplings, and juveniles between these two datasets, we chose not to model their dynamics.
...
The rationale behind this model is that, between two measurements, new individuals enter the population annually at a rate of $\phi$, and at every year, they survive to the next year given the probability $\rho$.
