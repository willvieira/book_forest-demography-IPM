  ...
  if(vitalRate == 'recruit') {
    ...
    td_sp |>
      ...
      mutate(
        ...
        ba_het = cov_perturbation(BA_comp_inter), 
        temp_nonScl = cov_perturbation(bio_01_mean),
        prec_nonScl = cov_perturbation(bio_12_mean),
        # scale perturbed climate variables
        temp = scale_clim(temp_nonScl, clim = 'temp', direction = 'scale'),
  ...
  if(vitalRate == 'growth') {
    td_sp |>
      mutate(
        ...
        # y_plot = growth_f(dbh0, deltaTime, BA_comp_sp, BA_comp_inter, bio_01_mean_scl, bio_12_mean_scl, r, plot, Lmax, Beta, theta, optimal_temp, tau_temp, optimal_prec, tau_prec),
    ...
    td_sp |>
      mutate(
        ...
        # growth for each perturbed covariate
        # y_plot = mort_f(dbh0, deltaTime, BA_comp_sp, BA_comp_inter, bio_01_mean_scl, bio_12_mean_scl, psi, plot, Beta, theta, optimal_temp, tau_temp, optimal_prec, tau_prec),
...
sampled_rows <- sample(1:4000, 1000)
...
map_dfr(
  ...
  ~map2_dfr(
    sampled_rows,
    .x,
    ~marginEffect(Sp = .y, rowID = .x, fec_dt, vitalRate = 'recruit', randomEffects = TRUE, at_range = NULL) |>
      bind_cols(species_id = .y)
  ),
  .progress = TRUE
 ) |>
...
### AME of temperature in the growth function over the latitude gradient

```{r plot_AME_growthVsLatitude,echo=Echo,eval=Eval,cache=Cache,warning=Warng,message=Msg}
#| label: fig-AMEsummTemp
#| fig-width: 9
#| fig-height: 5
#| fig-cap: "Average Marginal effect of Temperature in function of average latitudinal position of the species. Blue line is 50% quantile quantile and the gray lines are the 25% and 75% quantiles."
