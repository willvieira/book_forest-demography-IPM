...
We then increase the normalized values by a pertrubation size $p$, before transforming it back to the original scale:
...
Where $f$ denotes one of the growth, survival, or recruitment demographic models, $z$ the remaining covariates retained at their original values, and $\theta$ the collection of parameters.
This approach is valuable because it captures the locally linearized effect of each observation. 
Furthermore, computing the average effect across the observed dataset takes into account the diversity in the distribution of each covariate, giving more weight to regions where observations are more prevalent.
