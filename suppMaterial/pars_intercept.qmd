...
Because the growth rate intercepet decreases non-linearly with size and time derived from both the intrinsic growth rate $r$ and the maximum size $S_{max}$ parameters, we computed the average growth rate over a 10 years interval.
Also, because the size of the individual is integrated in the Von Bertalanffy growth model, we computed the 10 years growth starting from the lower size boundery of 12.7 cm.
...
pars_growth |>
  ...
  xlab('') +
  ylab('10 years average growth rate (mm/year)') +
  theme_classic() +
...
In the following figure we compare the maximum observed size in the literature with the maximum possible size parameter ($S_{max}$) where growth rate reaches zero.

```{r intcerpt_maxObsLmax,echo=Echo,eval=Eval,cache=Cache,warning=Warng,message=Msg}
#| fig-width: 6.5
#| fig-height: 5
#| fig-cap: "Correlation between maximum observed size in literature following @burns1990silvics and estimated parameter maximum size $S_{max}$"
...
Similar to the growth rate, the surival model has a temporal component in which survival probability decreases exponently with time.
...
pars_mort |>
  ...
  ylab('Maximum predicted age')
```


## Ingrowth rate

The ingrowth model has two intercepet components: the annual ingrowth rate per m$^2$ $m$ and the annual survival probability $p$.
For the second survival submodel we computed the 10 years survival probability of ingrowth individuals between measurement intervals.

```{r intcerpt_ingrowth,echo=Echo,eval=Eval,cache=Cache,warning=Warng,message=Msg}
#| fig-width: 8.5
#| fig-height: 5.5
#| fig-cap: "Intercept of the ingrwoth model for the number of individuals that ingress the population per year per m$^2$. Species are classified by their shade tolerance trait following @burns1990silvics"
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  pars_rec |>
    ...
    ggplot() +
    aes(succession, exp(mPop_log), fill = succession) +
    geom_boxplot() +
    scale_fill_manual(
      values = c('#f7fcb9', '#addd8e', '#31a354')
      # values = c("#87bc45", "#edbf33", "#ea5545")
    ...
    xlab('') +
    ylab('Annual ingrowth rate per m²') +
    theme_classic() +
    theme(legend.position = 'none') ->
  p2
