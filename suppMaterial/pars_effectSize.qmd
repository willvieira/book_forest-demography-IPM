...
Because of the non-linearity of the functions, we evaluate the condition effect of each covariate at the lower and upper observed values of the covariate-species combination.
...
This means that when we compare the conditional effect size between species, those effects do not account for the intercept value, nor it accounts for the other covariates.
Conditional effect size are values between 0 and 1, where 0 means no effect on the intercept.
...
For instance, the distribution of observed temperature for a specific species can be highly concentrated at the average, then while the conditional effect of temperature is relatively high at the lower border, it is not representative of the total population.
...
Second, we visualize the conditioanl effect size under the harsh conditions by selecting the strongest effect size between the lower and upper conditions.
