# Effect size of covariates {#sec-effectSize}

In this section, we quantify the relative effect size of each covariate (size, competition conspecific, competition heterospecific, temperature, and precipitation) on the vital rate intercepts.
We concentrate on the maximum and minimum range (what we refer to below as 1% and 99% quantile) for each covariate-species combination.
This is a kind of conditional effect at the extreme covariate ranges.
We discuss the average condition effect across all observations in the following chapter (@sec-averageCondEffect).
Specifically, we first compute the effect size at extreme range conditions for each covariate-species combination.
Second, visualize the effect size at harsh conditions by selecting the strongest effect size between the maximum and minimum range condition for each covariate-species combination.
...
map_dfr(
  ...
  select(r, Lmax, c(contains('optimal'), contains('tau'), 'Beta', 'theta')) |>
...
map_dfr(
  ...
  select(size_opt, size_var, c(contains('optimal'), contains('tau'), 'Beta', 'theta')) |>
  bind_cols(species_id = .x)
) ->
pars_survival
...
treeData |>
  ...
  summarise(
    ...
    bind_cols(myQuantile_f(BA_comp_intra, 'BAhet', probs_cov)),
...
pars_survival |>
  ...
  left_join(sp_range) |>
  mutate(
    across(contains('size_'), ~ sizeSurv_effect(.x, size_opt, size_var)),
    across(contains('temp_'), ~ clim_effect(.x, optimal_temp, tau_temp)),
    across(contains('prec_'), ~ clim_effect(.x, optimal_prec, tau_prec)),
    across(contains('BAcon_'), ~ BA_effect(.x, Beta)),
    across(contains('BAhet_'), ~ BA_effect(.x, Beta, theta))
...
treeData |>
  ...
  summarise(
    ...
    bind_cols(myQuantile_f(BA_comp_intra, 'BAhet', quant_probs)),
