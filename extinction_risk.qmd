...
db_sim_hot |>
  ...
  ylim(yLim) ->
p
ggsave('../talk_PhDdefence/images/lambda_vs_temp_abibal.png', p, width = 5.5, height = 3.5)


out_pars |>
  filter(species_id == Sp & border == 'cold' & sim == 2 & cond == 2) |>
  select(cond, iter, par, value) |>
  pivot_wider(names_from = par, values_from = value) |>
  group_by(cond, iter) |>
  expand_grid(
  ...
  bind_rows(
    out_pars |>
      filter(species_id == Sp & border == 'hot' & sim == 2 & cond == 2) |>
      select(cond, iter, par, value) |>
      pivot_wider(names_from = par, values_from = value) |>
      group_by(cond, iter) |>
      expand_grid(
  ...
  geom_point(
    data = db_sim_cold |>
      filter(cond == 2) |>
      filter(temp >= temp_range_cold[1] & temp <= temp_range_cold[2]) |>
      bind_cols(pos = 'Cold') |>
      bind_rows(
        db_sim_hot |>
          filter(cond == 2) |>
          filter(temp >= temp_range_hot[1] & temp <= temp_range_hot[2]) |>
          bind_cols(pos = 'Hot')
      ) |>
      group_by(pos) |>
      slice_sample(n = 1e4),
  ...
  ylim(yLim) ->
p
ggsave('../talk_PhDdefence/images/lambda_vs_temp_abibal_model.png', p, width = 6, height = 3.5)
...
out_pars |>
  ...
  ylim(yLim) ->
p
ggsave('../talk_PhDdefence/images/lambda_vs_temp_abibal_exp1.png', p, width = 5, height = 3.5)
```
...
plot_dt |>
  ...
  labs(
    y = expression('Suitable probability ('~Lambda~')'),
    x = "Relative position in the mean annual temperature gradient",
    color = NULL, fill = NULL
  ) +
  theme(
    legend.position = 'top',
  ...
  coord_cartesian(ylim = c(0, 1), clip = 'off') ->
p
ggsave('../talk_PhDdefence/images/suitable_probability_4position.png', p, width = 7, height = 3.5)
```
...
sp_dt |>
  ...
  select(species_id, cond, border, ext) |>
  group_by(species_id, border, cond) |>
  reframe(ext = mean(ext)) |>
  pivot_wider(
    names_from = border,
    values_from = ext
  ) |>
  ...
  pivot_longer(cols = c(Cold, Hot), names_to = 'border') |>
  # filter(species_id == '18032ABIBAL') |>
  left_join(
    sp_dt |> 
      filter(range_pos == 'border')  |>
      select(species_id, border, temp) |> 
      distinct(),
  ...
  aes(color = diff) +
  aes(shape = border) +
  # transition_states(cond, transition_length = 2, wrap = FALSE) +
  geom_point(aes(shape = border, fill = diff), size = 2.5, alpha = 0.8) +
...
p
ggsave(paste0('../talk_PhDdefence/images/suitable_probability_cond', Cond, '.png'), p, width = 6, height = 3.5)
}

animate(p, height = 3.5, width = 6, units = "in", res = 300)
anim_save(filename = '../talk_PhDdefence/images/suitable_probability.gif')
```
...
p
ggsave('../talk_PhDdefence/images/suitable_probability_comp.png', p, width = 7, height = 3.5)
```
