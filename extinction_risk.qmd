...
sp_dt |>
  ...
  scale_x_discrete(
    breaks = xLabel_order,
    labels = c('Cold border', 'Cold center', 'Hot center', 'Hot border')
  ) +
  labs(
    y = 'Suitable probability',
...
We further estimated the difference in suitable probability between the border and center (for climate) and between heterospecific and non-competition within each species.
...
The overall change in suitable probability for each condition is summarized in figure @fig-diff_sp_hist.
The effect of heterospecific competition on suitable probability was negative for most species across the cold and hot ranges.
For climate, however, the effect of changing temperature from the center to the border diverged between the cold and hot ranges.
While most species experienced a decrease in suitable probability toward the hot range, moving from the center toward the cold range, the pattern was divergent with larger increases and decreases in suitable probability.
Overall, the climate had a larger effect in changing suitable probability than competition.

```{r diff_sp_hist,echo=Echo,eval=Eval,cache=Cache,warning=Warng,message=Msg}
#| label: fig-diff_sp_hist
#| fig-width: 8
#| fig-height: 4
#| fig-cap: "Difference in suitable probability for climate and competition over the cold and hot ranges. When comparing one covariate, the classes of the other covariate are merged. For example, when we analyze the effect of climate, we see the distribution of the difference in suitable probability between the border and the center conditions, both for the no competition and heterospecific competition conditions together."
...
sp_dt |>
  ...
  mutate(
    ...
    diff_Cold_centerBorder_hc = Cold_2_border - Cold_2_center,
  ...
  mutate(
    sim = factor(sim, levels = c('comp', 'centerBorder'), labels = c('Competition\n(heter comp - no comp)', 'Climate\n(border - center)'))
  ) |>
  ...
  facet_wrap(~border) +
  ggridges::geom_density_ridges_gradient(alpha = 0.7, color = 'grey') +
  ...
  labs(
    x = 'Difference in suitable probability',
    y = NULL,
    fill = NULL
  )
```
...
We further tested whether the effect of climate and competition on suitable probability changes depends on the location of the species border.
Given that the negative effect of climate and competition was stronger at the hot range (@fig-diff_sp_hist), we can expect the strength of this effect to increase for species distributed at hotter temperatures.
...
Similar to @fig-diff_sp_hist, in @fig-sp_diff_over_MAT, we evaluate the difference in suitable probability for climate and competition across the mean annual temperature gradient.
While at the hot range, there is no apparent difference in climate and competition across the temperature gradient, at the cold range, the difference in suitable probability changed depending on the temperature range position.
For climate, as temperature reduced from the center to the border of the species, the suitable probability increased for species located at the hotter border but decreased for species close to the colder border.
When comparing suitable probability between heterospecific competition and no competition at the cold range, heterospecific competition reduced suitable probability only for species located at hotter temperatures.
...
#| fig-width: 8
#| fig-height: 6.5
#| fig-cap: "Difference in suitable probability for climate and competition at the cold and hot border for 31 species located over the mean annual temperature gradient. The difference in suitable probability for the climate effect is marginalized under the no-competition condition. Likewise, the effect of competition is marginalized at the border. In both cases, the marginalization of the other condition did not modify the interpretation of the figure. Species points are grouped by a Multivariate Normal Density function with 75% probability."
...
sp_dt |>
  ...
  mutate(
    diff_comp = range_border_2 - range_border_1,
  ...
  left_join(
    sp_dt |>
      ...
      reframe(temp = unique(temp))
  ) |>
  pivot_longer(
    cols = contains('diff')
  ) |>
  mutate(
    name = factor(name, levels = c('diff_clim', 'diff_comp'), labels = c('Climate (border - center)', 'Competition (heterospecific competition - no competition)'))
  ) |>
  ggplot() +
  aes(temp, value) +
