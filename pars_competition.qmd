...
pars_survival |>
  ...
  select(shade_sylvics, value, name) |>
  ...
  labs(
    fill = '',
    y = expression('Competition effect ('~beta~')'),
    x = 'Shade tolerance (Burns et al. 1990)',
    subtitle = 'Survival'
  ) ->
p
ggsave('../talk_PhDdefence/images/competition_vs_shade.png', p, width = 5.5, height = 4)
```
...
Their main finding is that CNDD is stronger in the tropics compared to temperate species, but only for low or intermediate abundant species.

```{r,echo=Echo,eval=Eval,cache=Cache,warning=Warng,message=Msg}
#| label: fig-CNDDvsLat
#| fig-width: 8.5
#| fig-height: 5
#| fig-cap: "Parameter controling density depense across the growth, survival, and recruitment model for 31 tree species. Each species is distributed across the latitude gradient using their median latitude across all plot observations."
