...
We show in @fig=ame the sensitivity of each species' population growth rate to conspecific and heterospecific competition, temperature, and precipitation.
...
At the hot range of the species, the sensitivity to competition decreased towards hotter temperatures, while the sensitivity to climate remained constant across the range of MAT.
...
#| fig-width: 8.5
#| fig-height: 4.5
#| fig-cap: "Relative sensitivity of species population growth rate between competition and climate for cold (blue), center (green), and hot (red) temperature ranges. We defined each species' temperature range position as the median Mean Annual Temperature across all observed plots for each cold, center, and hot range class. Species points are grouped by a Multivariate Normal Density function with 75% probability."
...
To quantify the sensitivity difference between competition and climate from @fig-sensBorder_temp, we analyze how the $CCR$ for the cold and hot range changes across the temperature range position of each species (@fig-ccrVsTemp).
We omitted the center range class to improve clarity, as $CCR$ did not change with MAT.
Overall, the relative sensitivity of $\lambda$ to climate than competition increases towards the cold and hot ranges.
While at the hot range the increase in sensitivity to climate was due to the decrease in sensitivity to competition, at the cold range it was due to a higher increase rate of climate than the competition (@fig-sensBorder_temp).
...
```{r ccrVsTemp,echo=Echo,eval=Eval,cache=Cache,warning=Warng,message=Msg}
#| label: fig-ccrVsTemp
#| fig-width: 7
#| fig-height: 4
#| fig-cap: "Relative sensitivity of species population growth rate between competition and climate for cold (blue), center (green), and hot (red) temperature ranges."
...
For plots with more than one observation, we averaged the each of the three metrics across the observations.
