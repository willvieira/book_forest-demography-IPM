...
AME_sp |>
  ...
  theme(legend.position = 'none') ->
p
ggsave('../talk_PhDdefence/images/fig-ame.png', p, width = 5, height = 4)
...
out |>
  ...
  ggplot() +
  aes(range_pos, e_size2, fill = border_cl, color = border_cl) +
  # ggdensity::geom_hdr(
  #   aes(color = NULL),
  #   probs = .7, alpha = .5,
  #   method = 'mvnorm'
  ...
  geom_smooth(method = "lm") +
  geom_point() +
  # geom_hline(yintercept = 0, alpha = 0.8, linetype = 2) +
  scale_fill_manual(values = c('#91bfdb', '#99d594', '#fc8d59')) +
...
out |>
  ...
  left_join(
    sp_range_pos |> 
      pivot_wider(names_from = border_cl, values_from = range_pos) |>
      mutate(range_size = abs(Cold - Hot))
  ) |>
  left_join(spIds, by = c('species_id' = 'species_id_new')) |>
  ggplot() +
  aes(Center, d_d) +
  facet_wrap(~covariable) +
  geom_smooth(method = 'lm') +
...
# define the two main species
sps <- c(
  dt_fig |>
    filter(covariable == 'Competition') |>
...
dt_fig |>
  mutate(covariable = factor(covariable, levels = c('Competition', 'Climate'))) |>
  filter(species_id %in% sps) |>
  mutate(value = if_else(covariable == 'Climate', NA, value)) |>
  ggplot() +
  aes(range_pos, value) +
  facet_wrap(~covariable) +
  geom_line(aes(color = diff, group = species_id)) +
  ...
  ylim(y_lim) ->
p
ggsave('../talk_PhDdefence/images/ame_hot_vs_coldA.png', p, width = 8, height = 4)
...
dt_fig |>
  ...
  ylim(y_lim) ->
p
ggsave('../talk_PhDdefence/images/ame_hot_vs_coldB.png', p, width = 8, height = 4)
...
dt_fig |>
  ...
  ylim(y_lim) ->
p
ggsave('../talk_PhDdefence/images/ame_hot_vs_coldC.png', p, width = 8, height = 4)
...
```{r ms_figure,echo=FALSE,eval=FALSE}
